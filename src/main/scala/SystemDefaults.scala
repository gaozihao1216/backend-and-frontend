...
import coursebackend.routes.ApiRouter
import coursebackend.services.admin.api.{AdminReviewService, ReviewSubmissionRequest, ReviewSubmissionResponse}
import coursebackend.services.admin.objects.ReviewedSubmission
import coursebackend.services.auth.api.{AuthService, BindBackendUserRequest, BindBackendUserResponse}
import coursebackend.services.auth.objects.BackendUser
import coursebackend.services.auth.tables.UserRow
import coursebackend.services.level.api.{DesignerLevelService, CreateLevelRequest, CreateLevelResponse, PlayerRatingService, RateLevelRequest, RateLevelResponse}
import coursebackend.services.level.objects._
import coursebackend.services.level.tables.{CommentRow, LevelRow, RatingRow, SubmissionRow}
import coursebackend.services.system.objects.{LevelStatus, SubmissionStatus, UserRole}
import coursebackend.services.user.api.{GetUserProfileRequest, GetUserProfileResponse, UserService}
import coursebackend.services.user.objects.{UserProfile, UserProfileStats}
...
object SystemDefaults {
  ...
  private var levels: Vector[LevelRow] = Vector(
    LevelRow(
      ...
      tags = List(coursebackend.services.system.objects.LevelTag.Beginner, coursebackend.services.system.objects.LevelTag.Puzzle),
      data = demoLevelData,
      authorId = "designer-1",
  ...
  levels = levels :+ LevelRow(
    ...
    tags = List(coursebackend.services.system.objects.LevelTag.Hard),
    data = demoLevelData,
    authorId = "designer-1",
