...
object AccessControl {
  ...
  def requireAdminLevel(connection: Connection, userId: String, adminLevel: AdminLevel): Step[UserRow] =
    PlanStep.fromEither(checkAdminLevel(connection, userId, adminLevel))

  /** 校验路由层 x-user-id 与 APIMessage.token 一致。 */
  def requireBoundIdentity(headerUserId: String, token: String): Step[Unit] =
    PlanStep.fromEither(checkBoundIdentity(headerUserId, token))

  /** 校验 userId 存在（不限 role）。 */
  def requireKnownUser(connection: Connection, userId: String): Step[UserRow] =
    PlanStep.fromEither(checkKnownUser(connection, userId))

  /** 同步角色校验，供测试与 `requireRole` 复用。 */
  ...
  /** 同步 admin 等级校验，供测试与 `requireAdminLevel` 复用。 */
  ...
  /** 同步用户存在校验，供测试与 `requireKnownUser` 复用。 */
  ...
  /** 同步身份绑定校验，供测试与 `requireBoundIdentity` 复用。 */
